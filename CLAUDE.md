...
- `ST_ENCOUNTER` (`Core/Battle/BattleEvents.lua` + `Core/Encounters/BossPhaseEvents.lua`) is a `/`-delimited tree keyed `"<Instance>/<Encounter>"`, auto-populated from `ENCOUNTER_START`. `Soundtrack.BattleEvents.RegisterEncounterKey` builds a key (registering every ancestor node first, which `GetFlattenedEventNodes` requires) and `PlayEncounterEvent` walks that key upward until it finds a node with tracks, falling back to `SOUNDTRACK_BOSS_BATTLE`. `Soundtrack.BossPhases` extends the tree one level deeper with `"<Instance>/<Encounter>/Stage N"` nodes driven by a boss mod: DBM's `DBM_SetStage` callback (`DBM:RegisterCallback`) or BigWigs' `BigWigs_SetStage` message (`BigWigsLoader.RegisterMessage(ownTable, ...)` — passing the loader itself errors). Both are subscribed when present but **DBM wins at dispatch** (`OnBigWigsSetStage` returns early while `attachedToDBM`), since the two would otherwise apply the same phase twice. Attachment is lazy: `Initialize()` plus a retry on every `ADDON_LOADED`, which keeps listening until *both* are attached because BigWigs_Core is load-on-demand. Stage and encounter events share `ST_BOSS_LVL`, so precedence is enforced explicitly: `PlayEncounterEvent` redirects to the active stage key when one is a child of the requested key, because boss mods can report stage 1 before `ENCOUNTER_START` arrives.
