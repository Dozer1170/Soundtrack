...
function Soundtrack.OptionsTab.SilenceDropDown_Initialize()
	...
	UIDropDownMenu_SetWidth(OptionsTab_SilenceDropDown, 110)
end

function Soundtrack.OptionsTab.SilenceDropDown_LoadSilences(silenceTexts)
	local currentSilence = SoundtrackUI.selectedSilence
	local info

	for i, silenceText in ipairs(silenceTexts) do
		local checked = nil
		if currentSilence == i then
			checked = 1
			UIDropDownMenu_SetText(OptionsTab_SilenceDropDown, silenceText)
		end

		info = {}
		info.text = silenceText
...
function Soundtrack.OptionsTab.ToggleAutoAddZones()
	SoundtrackAddon.db.profile.settings.AutoAddZones = not SoundtrackAddon.db.profile.settings.AutoAddZones
end

function Soundtrack.OptionsTab.ToggleAutoEscalateBattleMusic()
	SoundtrackAddon.db.profile.settings.EscalateBattleMusic =
		not SoundtrackAddon.db.profile.settings.EscalateBattleMusic
end

function Soundtrack.OptionsTab.ToggleEnableZoneMusic()
	SoundtrackAddon.db.profile.settings.EnableZoneMusic = not SoundtrackAddon.db.profile.settings.EnableZoneMusic
end

function Soundtrack.OptionsTab.ToggleEnableBattleMusic()
	SoundtrackAddon.db.profile.settings.EnableBattleMusic = not SoundtrackAddon.db.profile.settings.EnableBattleMusic
end

function Soundtrack.OptionsTab.ToggleEnableMiscMusic()
	SoundtrackAddon.db.profile.settings.EnableMiscMusic = not SoundtrackAddon.db.profile.settings.EnableMiscMusic
end
...
function Soundtrack.OptionsTab.ToggleLockPlaybackControls()
	SoundtrackAddon.db.profile.settings.LockPlaybackControls =
		not SoundtrackAddon.db.profile.settings.LockPlaybackControls
end

function Soundtrack.OptionsTab.ToggleHidePlaybackButtons()
