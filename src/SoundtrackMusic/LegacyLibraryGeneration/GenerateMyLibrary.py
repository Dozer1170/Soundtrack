...
def GetTrackInfo(filePath, recursionCount = 0):
    ...
    try:
        ...
        try:
            ...
            print "Unhandled Exception in %s while %s, trying again" % (filePath, op);
            if pauseOnError == 'true' and recursionCount > 10:
                raw_input("Press ENTER to skip track and continue...")
            else:
                return GetTrackInfo(filePath, recursionCount + 1)
        except UnicodeEncodeError:
            unicodeError = unicodeError + 1
            print " unprintable filename";
...
def main(argv, stdout, environ):
    ...
    workingPath = workingPath.replace('/LegacyLibraryGeneration', '')
    mp3Path = workingPath.replace('\\', '/')
    scriptFilePath = mp3Path + '/MyTracks.lua'
